...
export class ProfileService {
  ...
  private prepareAvatarUpload(
    ...
    if (!this.isValidAvatarBuffer(ext, file.buffer)) {
      throw new Error(`Invalid avatar image content for .${ext}`)
    }
  ...
  private isValidAvatarBuffer(ext: string, buffer: Buffer): boolean {
    switch (ext.toLowerCase()) {
      case 'png':
